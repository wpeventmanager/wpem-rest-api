...
abstract class WPEM_REST_Posts_Controller extends WPEM_REST_Controller {
	...
	public function create_item_permissions_check( $request ) {
		if ( ! wpem_rest_check_post_permissions( $this->post_type, 'create' ) ) {
			return new WP_Error( 'wpem_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'wp-event-manager-rest-api' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function batch_items_permissions_check( $request ) {
		if ( ! wpem_rest_check_post_permissions( $this->post_type, 'batch' ) ) {
			return new WP_Error( 'wpem_rest_cannot_batch', __( 'Sorry, you are not allowed to batch manipulate this resource.', 'wp-event-manager-rest-api' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new WP_Error( "wpem_rest_invalid_{$this->post_type}_id", __( 'Invalid ID.', 'wp-event-manager-rest-api' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new WP_Error( "wpem_rest_{$this->post_type}_invalid_id", __( 'ID is invalid.', 'wp-event-manager-rest-api' ), array( 'status' => 400 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new WP_Error( "wpem_rest_{$this->post_type}_invalid_id", __( 'ID is invalid.', 'wp-event-manager-rest-api' ), array( 'status' => 404 ) );
		}
