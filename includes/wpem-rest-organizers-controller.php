...
class WPEM_REST_Organizers_Controller extends WPEM_REST_CRUD_Controller
{
    ...
    public function wpem_prepare_object_for_response($object, $request)
    {
        ...
        $data = $this->wpem_get_organizer_data($object, $context);
     ...
     * @return array
     */
    protected function wpem_get_organizer_data($organizer, $context = 'view')
    {
    ...
    public function wpem_prepare_item_for_response($post, $request)
    {
        ...
        $data = $this->wpem_get_organizer_data($organizer, $context);
        $data = $this->add_additional_fields_to_object($data, $request);
        $data = $this->filter_response_by_context($data, $context);

        $response = rest_ensure_response($data);
        $response->add_links($this->wpem_prepare_links($organizer, $request));
