<?php
/**
 * REST API Venues controller
 *
 * Handles requests to the /venues endpoint.
 *
 * @since 1.0.0
 */
...
/**
 * REST API Venues controller class.
 *
 * @extends WPEM_REST_CRUD_Controller
 */
class WPEM_REST_Venues_Controller extends WPEM_REST_CRUD_Controller
{
    ...
    /**
     * Prepare a single venue output for response.
     *
     ...
     * Get venue data.
     *
     * @param WP_Post $venue   Venue post object.
    ...
    protected function get_venue_data($venue, $context = 'view')
    {
        ...
        $image_src = '';
        $thumbnail_id = get_post_thumbnail_id( $venue->ID );
        if ( $thumbnail_id ) {
        ...
        $data = array(
            ...
            'featured'      => (bool) get_post_meta($venue->ID, '_featured', true),
            'description'   => $description,
            'image'         => $image_src,
            'meta_data'     => get_post_meta($venue->ID),
        );
    ...
    /**
     * Prepare a single venue output for response (legacy method).
     *
    ...
    protected function prepare_object_for_database($request, $creating = false)
    {
        ...
        if ( $creating ) {
            ...
            $post = get_post($id);
            if ( ! $post || $post->post_type !== $this->post_type ) {
                return new WP_Error('wpem_rest_invalid_id', __('Invalid venue ID.', 'wpem-rest-api'), array('status' => 404));
            }
    ...
    /**
     * Get the Venue's schema, conforming to JSON Schema.
     *
     * @return array
     */
    public function get_item_schema()
    {
        $schema = array(
            ...
            'properties' => array(
                ...
                'name' => array(
                    'description' => __('Venue name.', 'wpem-rest-api'),
                    'type'        => 'string',
                    'context'     => array('view', 'edit'),
                ),
                'slug' => array(
                    'description' => __('Venue slug.', 'wpem-rest-api'),
                    'type'        => 'string',
                    'context'     => array('view', 'edit'),
                ),
                'permalink' => array(
                    'description' => __('Venue URL.', 'wpem-rest-api'),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => array('view', 'edit'),
                    'readonly'    => true,
                ...
                'date_created' => array(
                    'description' => __("The date the venue was created, in the site's timezone.", 'wpem-rest-api'),
                    'type'        => 'date-time',
                    'context'     => array('view', 'edit'),
                    'readonly'    => true,
                ),
                'date_modified' => array(
                    'description' => __("The date the venue was last modified, in the site's timezone.", 'wpem-rest-api'),
                    'type'        => 'date-time',
                    'context'     => array('view', 'edit'),
                    'readonly'    => true,
                ),
                'status' => array(
                    'description' => __('Venue status (post status).', 'wpem-rest-api'),
                    'type'        => 'string',
                    'default'     => 'publish',
                ...
                'featured' => array(
                    'description' => __('Featured venue.', 'wpem-rest-api'),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array('view', 'edit'),
                ),
                'description' => array(
                    'description' => __('Venue description.', 'wpem-rest-api'),
                    'type'        => 'string',
                    'context'     => array('view', 'edit'),
                ),
                'image' => array(
                    'description' => __('Venue featured image URL.', 'wpem-rest-api'),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => array('view', 'edit'),
                ),
    ...
    public function get_collection_params()
    {
        ...
        $params['slug'] = array(
            'description'       => __('Limit result set to venues with a specific slug.', 'wpem-rest-api'),
            'type'              => 'string',
            'validate_callback' => 'rest_validate_request_arg',
        );
        $params['status'] = array(
            'default'           => 'any',
            'description'       => __('Limit result set to venues assigned a specific status.', 'wpem-rest-api'),
            'type'              => 'string',
...
new WPEM_REST_Venues_Controller();