			...
			<div id="wpbody-content" class="wpem-admin-container">
				<div class="wpem-wrap">
					<form method="post" name="wpem-rest-settings-form" <?php echo esc_attr($action); ?>>
							...
							<ul class="wpem-admin-left-menu">
								<?php foreach ($this->settings as $key => $section) { ?>
									<li class="wpem-admin-left-menu-item">
										<a class="wpem-icon-<?php echo isset($section['icon']) ? esc_attr($section['icon']) : 'meter'; ?> nav-tab <?php echo ($current_tab === $key) ? 'nav-tab-active' : ''; ?>"
						...
						?>
						<div
							class="wpem-admin-right-container wpem-<?php echo esc_html($current_tab); ?> wpem-app-branding-mode <?php if ($current_tab == 'app-branding') {
									  echo esc_attr($class);
								  } ?>">
							<div class="metabox-holder wpem-admin-right-container-holder">
								...
								<p class="submit">
									<input type="submit" class="button-primary wpem-backend-theme-button" id="save-changes"
										value="<?php esc_html_e('Save Changes', 'wpem-rest-api'); ?>" />
								</p>
							</div>
						</div>
					</form>
