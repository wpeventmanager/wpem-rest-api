...
class WPEM_API_Keys_Table_List extends WP_List_Table {
	...
	public function column_title( $key ) {
		$url     =  admin_url( 'edit.php?post_type=event_listing&page=wpem-rest-api-key-settings&edit-key=' . $key['key_id'] );
			...
			$actions['trash'] = '<a class="submitdelete" aria-label="' . esc_attr__( 'Revoke API key', 'wp-event-manager-rest-api' ) . '" href="' . esc_url(
				wp_nonce_url(
					add_query_arg(
						...
						admin_url( 'edit.php?post_type=event_listing&page=wpem-rest-api-key-settings' )
					),
					'revoke'
				)
